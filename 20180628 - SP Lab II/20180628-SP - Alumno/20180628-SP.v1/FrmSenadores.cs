        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    xml.Guardar(AppDomain.CurrentDomain.BaseDirectory + "Resultados xml", votacion);
                    Dao dao = new Dao();
                    //Dao<Votacion> dao = new Dao<Votacion>();
                    StringBuilder cadena = new StringBuilder();
                    cadena.Append("server=DELL-PC\\SQLEXPRESS; database=Votacion;Integrated Security=True");
                    //"Data Source=[Instancia Del Servidor]; Initial Catalog =[Nombre de la Base de Datos]; Integrated Security = True"
                    dao.Guardar(cadena.ToString(), votacion);
                }
            }
        }
